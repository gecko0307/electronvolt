...
struct Frustum
{
    ...
    bool containsBsphere(Sphere bsphere, bool checkNearPlane = false)
    {
	    float distance;

        distance = leftPlane.distance(bsphere.center);
        if (distance <= -bsphere.radius)
            return false;

        distance = rightPlane.distance(bsphere.center);
        if (distance <= -bsphere.radius)
            return false;

        distance = bottomPlane.distance(bsphere.center);
        if (distance <= -bsphere.radius)
            return false;

        distance = topPlane.distance(bsphere.center);
        if (distance <= -bsphere.radius)
            return false;

        distance = farPlane.distance(bsphere.center);
        if (distance <= -bsphere.radius)
            return false;
        ...
        distance = nearPlane.distance(bsphere.center);
        if (distance <= -bsphere.radius)
            return false;
