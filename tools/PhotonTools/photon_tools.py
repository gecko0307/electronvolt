bl_info = {
    "name": "Photon Content Creation Tools",
    "author": "Timur Gafarov (Gecko)",
    "version": (1, 0),
    "blender": (2, 6, 4),
    "location": "File > Export > Photon Engine Data File (.dat)",
    "description": "Tools to create and export Photon Engine content",
    "warning": "",
    "wiki_url": "",
    "tracker_url": "",
...
def doExport(context, filepath=""):
    print("Exporting selected objects as Photon Engine Data File...")
    ...
    for obj in objects:
        ...
        if (obj.type == 'EMPTY' and obj['PhType'] == 'spawnpos'):
        ...
        if (obj.type == 'EMPTY' and obj['PhType'] == 'collectible'):
            collectibleData = bytearray()

            collectibleType = 0
            if 'PhCollectibleType' in obj:
                collectibleType = obj['PhCollectibleType']
    ...
    for matName, matIndex in materials.items():
        ...
        if 'PhDanger' in mat:
            danger = mat['PhDanger']
        if 'PhWalkSound' in mat:
            walkSound = mat['PhWalkSound']
...
class PhotonAddObject(bpy.types.Operator):
    bl_idname = ("scene.photon_add_object")
    bl_label = ("Photon Object :: add object")
...
def menu_func_add_photon_object(self, context):
    self.layout.operator_menu_enum(PhotonAddObject.bl_idname, property="value", text="Photon", icon='LOGIC')
    
def menu_func_export_photon_map(self, context):
    self.layout.operator(ExportPhotonDataFile.bl_idname, text = "Photon Engine Data File (.dat)")
...
class PhotonPanel(bpy.types.Panel):
    bl_label = "Photon Engine Tools"
...
def register():
    ...
    bpy.types.INFO_MT_add.append(menu_func_add_photon_object)
    bpy.types.INFO_MT_file_export.append(menu_func_export_photon_map)

def unregister():
    bpy.utils.unregister_class(PhotonPanel)
    bpy.utils.unregister_class(OpBakingModeOn)
    bpy.utils.unregister_class(OpBakingModeOff)
    bpy.types.INFO_MT_add.remove(menu_func_add_photon_object)
    bpy.types.INFO_MT_file_export.remove(menu_func_export_photon_map)
